...
program
  ...
  .action(async (mission: string | undefined, options: CLIOptions & { shokunin?: string; profile?: string; config?: string }) => {
 ...
 * 處理斜線指令安裝
 */
async function handleSetupCommands(force = false, global = false): Promise<void> {
  try {
    const { spawn } = await import('child_process');
 ...
 * 處理斜線指令移除
 */
async function handleRemoveCommands(global = false): Promise<void> {
  try {
    const { spawn } = await import('child_process');
    const { resolve } = await import('path');
    const { fileURLToPath } = await import('url');
 ...
 * 處理列出風格
 */
async function handleListStyles(): Promise<void> {
